...
class GameService {
  public async joinGameRoom(socket: Socket, roomId: string, username: string, isScrumMaster: boolean): Promise<any> {
    return new Promise((rs, rj) => {
      socket.emit("join_game", { roomId, username, isScrumMaster });
      socket.on("scrum_master_joined", () => rs({ role: 'scrumMaster' }));
      socket.on("participant_joined", () => rs({ role: 'participant' }))
